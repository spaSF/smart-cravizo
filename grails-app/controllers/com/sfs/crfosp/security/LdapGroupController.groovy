...
class LdapGroupController {
	...
	def index() {
		respond response:responseSCService.gridResponse(LdapGroup.class,params)
	}

	@Transactional
	def save() {
		LdapGroup instance = LdapGroup.newInstance(params)
		...
		LdapGroup.withTransaction {status->
			try {
	...
	def update() {
		LdapGroup instance
		try {
			instance = LdapGroup.lock(params.id)
