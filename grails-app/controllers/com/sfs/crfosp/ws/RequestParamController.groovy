...
class RequestParamController {
	...
	def index() {
		...
		respond response:responseSCService.gridResponse(RequestParam.class,params)
	}

	@Transactional
	def save() {
		RequestParam instance = RequestParam.newInstance(params)
		...
		RequestParam.withTransaction {status->
			try {
	...
	@Transactional
	def update() {
		RequestParam instance = RequestParam.lock(params.id)
		if (!instance) {
			respond response:[status:-1,data:message(code:"default.not.found.message",args:[
					message(code:"requestParam.label"),
					params.id
				])]
			return
		}
