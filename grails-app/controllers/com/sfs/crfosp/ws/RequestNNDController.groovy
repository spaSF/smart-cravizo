...
class RequestNNDController {
	...
	def index() {
		respond response:responseSCService.gridResponse(RequestNND.class,params)
	}

	@Transactional
	def save() {
		RequestNND instance = RequestNND.newInstance(params)
		...
		RequestNND.withTransaction {status->
			try {
	...
	def update() {
		RequestNND instance
		try {
			instance = RequestNND.lock(params.id)
