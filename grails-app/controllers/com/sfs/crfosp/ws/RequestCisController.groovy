...
class RequestCisController {
	...
	def index() {
		respond response:responseSCService.gridResponse(RequestCis.class,params)
	}

	@Transactional
	def save() {
		RequestCis instance = RequestCis.newInstance(params)
		...
		RequestCis.withTransaction {status->
			try {
	...
	def update() {
		RequestCis instance
		try {
			instance = RequestCis.lock(params.id)
