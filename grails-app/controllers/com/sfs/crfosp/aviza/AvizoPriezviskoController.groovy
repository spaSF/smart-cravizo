...
class AvizoPriezviskoController {
	...
	def index() {
		respond response:responseSCService.gridResponse(AvizoPriezvisko.class,params)
	}

	@Transactional
	def save() {
		AvizoPriezvisko instance = AvizoPriezvisko.newInstance(params)
		...
		AvizoPriezvisko.withTransaction {status->
			try {
	...
	@Transactional
	def update() {
		AvizoPriezvisko instance
		try {
			instance = AvizoPriezvisko.lock(params.id)
		...
		if (!instance) {
			respond response:[status:-1,data:message(code:"default.not.found.message",args:[
					message(code:"avizoPriezvisko.label"),
					params.id
				])]
			return
		}
