...
class AvizoController {
	...
	def create(){
		render view:"newAvizo",model:[tabSet:params.tabSet,dataSource:params.dataSource]
	}

	def detail(){
		def mpars = params
		Avizo instance = Avizo.get(params.id)
		if (!instance) {
			respond response:[status:-1,data:message(code:"default.not.found.message",args:[
					message(code:"avizo.label"),
					params.id
				])]
			return
		}
		String detailDS = "AvizoDetail"
		render view:"detail",model:[tabSet:params.tabSet,dataSource:detailDS,instanceId:instance.id]
	}
	
	def index() {
		encrypterService.authCrypter()
		respond response:responseSCService.gridResponse(Avizo.class,params)
	}

	@Transactional
	def save() {
		Avizo instance = Avizo.newInstance(params)
