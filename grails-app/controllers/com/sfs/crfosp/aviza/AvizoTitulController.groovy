...
class AvizoTitulController {
	...
	def index() {
		respond response:responseSCService.gridResponse(AvizoTitul.class,params)
	}

	@Transactional
	def save() {
		AvizoTitul instance = AvizoTitul.newInstance(params)
		...
		AvizoTitul.withTransaction {status->
			try {
	...
	def update() {
		AvizoTitul instance
		try {
			instance = AvizoTitul.lock(params.id)
