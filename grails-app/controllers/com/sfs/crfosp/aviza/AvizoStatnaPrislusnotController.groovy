...
class AvizoStatnaPrislusnotController {
	...
	def index() {
		respond response:responseSCService.gridResponse(AvizoStatnaPrislusnot.class,params)
	}

	@Transactional
	def save() {
		AvizoStatnaPrislusnot instance = AvizoStatnaPrislusnot.newInstance(params)
		...
		AvizoStatnaPrislusnot.withTransaction {status->
			try {
	...
	@Transactional
	def update() {
		AvizoStatnaPrislusnot instance
		try {
			instance = AvizoStatnaPrislusnot.lock(params.id)
		...
		if (!instance) {
			respond response:[status:-1,data:message(code:"default.not.found.message",args:[
					message(code:"avizoStatnaPrislusnot.label"),
					params.id
				])]
			return
		}
