...
class AvizoMenoController {
	...
	def index() {
		respond response:responseSCService.gridResponse(AvizoMeno.class,params)
	}

	@Transactional
	def save() {
		AvizoMeno instance = AvizoMeno.newInstance(params)
		...
		AvizoMeno.withTransaction {status->
			try {
	...
	def update() {
		AvizoMeno instance
		try {
			instance = AvizoMeno.lock(params.id)
