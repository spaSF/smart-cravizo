...
class AvizoRodnePriezController {
	...
	def index() {
		respond response:responseSCService.gridResponse(AvizoRodnePriez.class,params)
	}

	@Transactional
	def save() {
		AvizoRodnePriez instance = AvizoRodnePriez.newInstance(params)
		...
		AvizoRodnePriez.withTransaction {status->
			try {
	...
	@Transactional
	def update() {
		AvizoRodnePriez instance
		try {
			instance = AvizoRodnePriez.lock(params.id)
		...
		if (!instance) {
			respond response:[status:-1,data:message(code:"default.not.found.message",args:[
					message(code:"avizoRodnePriez.label"),
					params.id
				])]
			return
		}
