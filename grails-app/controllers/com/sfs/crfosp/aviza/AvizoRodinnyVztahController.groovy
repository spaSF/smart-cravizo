...
class AvizoRodinnyVztahController {
	...
	def index() {
		...
		respond response:responseSCService.gridResponse(AvizoRodinnyVztah.class,params)
	}

	@Transactional
	def save() {
		AvizoRodinnyVztah instance = AvizoRodinnyVztah.newInstance(params)
		...
		AvizoRodinnyVztah.withTransaction {status->
			try {
	...
	@Transactional
	def update() {
		AvizoRodinnyVztah instance
		try {
			instance = AvizoRodinnyVztah.lock(params.id)
		...
		if (!instance) {
			respond response:[status:-1,data:message(code:"default.not.found.message",args:[
					message(code:"avizoRodinnyVztah.label"),
					params.id
				])]
			return
		}
