...
class AvizoAtributyController {
	...
	def index() {
		...
		respond response:responseSCService.gridResponse(AvizoAtributy.class,params)
	}

	@Transactional
	def save() {
		AvizoAtributy instance = AvizoAtributy.newInstance(params)
		...
		AvizoAtributy.withTransaction {status->
			try {
	...
	@Transactional
	def update() {
		AvizoAtributy instance
		try {
			instance = AvizoAtributy.lock(params.id)
		...
		if (!instance) {
			respond response:[status:-1,data:message(code:"default.not.found.message",args:[
					message(code:"avizoAtributy.label"),
					params.id
				])]
			return
		}
