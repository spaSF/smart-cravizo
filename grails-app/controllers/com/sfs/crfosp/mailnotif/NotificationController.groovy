...
class NotificationController {
	...
	def index() {
		respond response:responseSCService.gridResponse(Notification.class,params)
	}

	def sendTestMail(){
		String mfrom=params.from
		String mto=params.to
		sendMail{
			to mto
			from mfrom
			subject "Test mail"
			body "TEST GRAILS MAIL PLUGIN"
		}
		
		respond response:[status:0]
	}
	
	@Transactional
	def save() {
		Notification instance = Notification.newInstance(params)
	...
	def update() {
		...
		try {
			instance = Notification.lock(params.id)
		...
		if (!instance) {
			respond response:[status:-1,data:message(code:"default.not.found.message",args:[
					message(code:"notification.label"),
					params.id
				])]
			return
		}
