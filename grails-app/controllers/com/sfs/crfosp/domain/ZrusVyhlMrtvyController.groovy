...
class ZrusVyhlMrtvyController {
	...
	def index() {
		...
		respond response:responseSCService.gridResponse(ZrusVyhlMrtvy.class,params)
	}

	@Transactional
	def save() {
		ZrusVyhlMrtvy instance = ZrusVyhlMrtvy.newInstance(params)
		...
		ZrusVyhlMrtvy.withTransaction {status->
			try {
	...
	@Transactional
	def update() {
		ZrusVyhlMrtvy instance = ZrusVyhlMrtvy.lock(params.id)
		if (!instance) {
			respond response:[status:-1,data:message(code:"default.not.found.message",args:[
					message(code:"zrusVyhlMrtvy.label"),
					params.id
				])]
			return
		}
