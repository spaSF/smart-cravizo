...
class PobytRegionController {
	...
	def index() {
		...
		respond response:responseSCService.gridResponse(PobytRegion.class,params)
	}

	@Transactional
	def save() {
		PobytRegion instance = PobytRegion.newInstance(params)
		...
		PobytRegion.withTransaction {status->
			try {
	...
	@Transactional
	def update() {
		PobytRegion instance = PobytRegion.lock(params.id)
		if (!instance) {
			respond response:[status:-1,data:message(code:"default.not.found.message",args:[
					message(code:"pobytRegion.label"),
					params.id
				])]
			return
		}
