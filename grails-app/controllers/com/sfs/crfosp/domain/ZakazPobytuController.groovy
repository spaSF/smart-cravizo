...
class ZakazPobytuController {
	...
	def index() {
		...
		respond response:responseSCService.gridResponse(ZakazPobytu.class,params)
	}

	@Transactional
	def save() {
		ZakazPobytu instance = ZakazPobytu.newInstance(params)
		...
		ZakazPobytu.withTransaction {status->
			try {
	...
	@Transactional
	def update() {
		ZakazPobytu instance = ZakazPobytu.lock(params.id)
		if (!instance) {
			respond response:[status:-1,data:message(code:"default.not.found.message",args:[
					message(code:"zakazPobytu.label"),
					params.id
				])]
			return
		}
