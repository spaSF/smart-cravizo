...
class RodnePriezviskoController {
	...
	def index() {
		...
		respond response:responseSCService.gridResponse(RodnePriezvisko.class,params)
	}

	@Transactional
	def save() {
		RodnePriezvisko instance = RodnePriezvisko.newInstance(params)
		...
		RodnePriezvisko.withTransaction {status->
			try {
	...
	@Transactional
	def update() {
		RodnePriezvisko instance = RodnePriezvisko.lock(params.id)
		if (!instance) {
			respond response:[status:-1,data:message(code:"default.not.found.message",args:[
					message(code:"rodnePriezvisko.label"),
					params.id
				])]
			return
		}
