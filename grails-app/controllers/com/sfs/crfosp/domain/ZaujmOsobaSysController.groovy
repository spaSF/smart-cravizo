...
class ZaujmOsobaSysController {
	...
	def index() {
		...
		respond response:responseSCService.gridResponse(ZaujmOsobaSys.class,params)
	}

	@Transactional
	def save() {
		ZaujmOsobaSys instance = ZaujmOsobaSys.newInstance(params)
		...
		ZaujmOsobaSys.withTransaction {status->
			try {
	...
	@Transactional
	def update() {
		ZaujmOsobaSys instance = ZaujmOsobaSys.lock(params.id)
		if (!instance) {
			respond response:[status:-1,data:message(code:"default.not.found.message",args:[
					message(code:"zaujmOsobaSys.label"),
					params.id
				])]
			return
		}
