...
class RodinnyVztahController {
	...
	def index() {
		...
		respond response:responseSCService.gridResponse(RodinnyVztah.class,params)
	}

	@Transactional
	def save() {
		RodinnyVztah instance = RodinnyVztah.newInstance(params)
		...
		RodinnyVztah.withTransaction {status->
			try {
	...
	@Transactional
	def update() {
		RodinnyVztah instance = RodinnyVztah.lock(params.id)
		if (!instance) {
			respond response:[status:-1,data:message(code:"default.not.found.message",args:[
					message(code:"rodinnyVztah.label"),
					params.id
				])]
			return
		}
