...
class ObmPravnejSposController {
	...
	def index() {
		...
		respond response:responseSCService.gridResponse(ObmPravnejSpos.class,params)
	}

	@Transactional
	def save() {
		ObmPravnejSpos instance = ObmPravnejSpos.newInstance(params)
		...
		ObmPravnejSpos.withTransaction {status->
			try {
	...
	@Transactional
	def update() {
		ObmPravnejSpos instance = ObmPravnejSpos.lock(params.id)
		if (!instance) {
			respond response:[status:-1,data:message(code:"default.not.found.message",args:[
					message(code:"obmPravnejSpos.label"),
					params.id
				])]
			return
		}
