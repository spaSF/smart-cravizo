...
class StatnaPrislusnotController {
	...
	def index() {
		...
		respond response:responseSCService.gridResponse(StatnaPrislusnot.class,params)
	}

	@Transactional
	def save() {
		StatnaPrislusnot instance = StatnaPrislusnot.newInstance(params)
		...
		StatnaPrislusnot.withTransaction {status->
			try {
	...
	@Transactional
	def update() {
		StatnaPrislusnot instance = StatnaPrislusnot.lock(params.id)
		if (!instance) {
			respond response:[status:-1,data:message(code:"default.not.found.message",args:[
					message(code:"statnaPrislusnot.label"),
					params.id
				])]
			return
		}
