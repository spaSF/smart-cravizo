...
class StavDomVchodCisController {
	...
	def index() {
		respond response:responseSCService.gridResponse(StavDomVchodCis.class,params)
	}

	@Transactional
	def save() {
		StavDomVchodCis instance = StavDomVchodCis.newInstance(params)
		...
		StavDomVchodCis.withTransaction {status->
			try {
	...
	@Transactional
	def update() {
		StavDomVchodCis instance = StavDomVchodCis.lock(params.id)
		if (!instance) {
			respond response:[status:-1,data:message(code:"default.not.found.message",args:[
					message(code:"stavDomVchodCis.label"),
					params.id
				])]
			return
		}
