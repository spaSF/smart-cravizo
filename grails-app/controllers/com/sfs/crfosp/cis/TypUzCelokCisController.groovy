...
class TypUzCelokCisController {
	...
	def index() {
		respond response:responseSCService.gridResponse(TypUzCelokCis.class,params)
	}

	@Transactional
	def save() {
		TypUzCelokCis instance = TypUzCelokCis.newInstance(params)
		...
		TypUzCelokCis.withTransaction {status->
			try {
	...
	@Transactional
	def update() {
		TypUzCelokCis instance = TypUzCelokCis.lock(params.id)
		if (!instance) {
			respond response:[status:-1,data:message(code:"default.not.found.message",args:[
					message(code:"typUzCelokCis.label"),
					params.id
				])]
			return
		}
