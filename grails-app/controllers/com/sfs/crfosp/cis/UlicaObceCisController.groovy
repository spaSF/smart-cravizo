...
class UlicaObceCisController {
	...
	def index() {
		respond response:responseSCService.gridResponse(UlicaObceCis.class,params)
	}

	@Transactional
	def save() {
		UlicaObceCis instance = UlicaObceCis.newInstance(params)
		...
		UlicaObceCis.withTransaction {status->
			try {
	...
	@Transactional
	def update() {
		UlicaObceCis instance = UlicaObceCis.lock(params.id)
		if (!instance) {
			respond response:[status:-1,data:message(code:"default.not.found.message",args:[
					message(code:"ulicaObceCis.label"),
					params.id
				])]
			return
		}
