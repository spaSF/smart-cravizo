...
class TypVztahCisController {
	...
	def index() {
		respond response:responseSCService.gridResponse(TypVztahCis.class,params)
	}

	@Transactional
	def save() {
		TypVztahCis instance = TypVztahCis.newInstance(params)
		...
		TypVztahCis.withTransaction {status->
			try {
	...
	@Transactional
	def update() {
		TypVztahCis instance = TypVztahCis.lock(params.id)
		if (!instance) {
			respond response:[status:-1,data:message(code:"default.not.found.message",args:[
					message(code:"typVztahCis.label"),
					params.id
				])]
			return
		}
