...
class RegUlicaCisController {
	...
	def index() {
		respond response:responseSCService.gridResponse(RegUlicaCis.class,params)
	}

	@Transactional
	def save() {
		RegUlicaCis instance = RegUlicaCis.newInstance(params)
		...
		RegUlicaCis.withTransaction {status->
			try {
	...
	@Transactional
	def update() {
		RegUlicaCis instance = RegUlicaCis.lock(params.id)
		if (!instance) {
			respond response:[status:-1,data:message(code:"default.not.found.message",args:[
					message(code:"regUlicaCis.label"),
					params.id
				])]
			return
		}
