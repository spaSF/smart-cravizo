...
class StupenZverejCisController {
	...
	def index() {
		respond response:responseSCService.gridResponse(StupenZverejCis.class,params)
	}

	@Transactional
	def save() {
		StupenZverejCis instance = StupenZverejCis.newInstance(params)
		...
		StupenZverejCis.withTransaction {status->
			try {
	...
	@Transactional
	def update() {
		StupenZverejCis instance = StupenZverejCis.lock(params.id)
		if (!instance) {
			respond response:[status:-1,data:message(code:"default.not.found.message",args:[
					message(code:"stupenZverejCis.label"),
					params.id
				])]
			return
		}
