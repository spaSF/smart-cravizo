...
class StavDomuCisController {
	...
	def index() {
		respond response:responseSCService.gridResponse(StavDomuCis.class,params)
	}

	@Transactional
	def save() {
		StavDomuCis instance = StavDomuCis.newInstance(params)
		...
		StavDomuCis.withTransaction {status->
			try {
	...
	@Transactional
	def update() {
		StavDomuCis instance = StavDomuCis.lock(params.id)
		if (!instance) {
			respond response:[status:-1,data:message(code:"default.not.found.message",args:[
					message(code:"stavDomuCis.label"),
					params.id
				])]
			return
		}
