...
class TypObmSposCisController {
	...
	def index() {
		respond response:responseSCService.gridResponse(TypObmSposCis.class,params)
	}

	@Transactional
	def save() {
		TypObmSposCis instance = TypObmSposCis.newInstance(params)
		...
		TypObmSposCis.withTransaction {status->
			try {
	...
	@Transactional
	def update() {
		TypObmSposCis instance = TypObmSposCis.lock(params.id)
		if (!instance) {
			respond response:[status:-1,data:message(code:"default.not.found.message",args:[
					message(code:"typObmSposCis.label"),
					params.id
				])]
			return
		}
