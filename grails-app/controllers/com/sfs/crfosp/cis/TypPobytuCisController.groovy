...
class TypPobytuCisController {
	...
	def index() {
		respond response:responseSCService.gridResponse(TypPobytuCis.class,params)
	}

	@Transactional
	def save() {
		TypPobytuCis instance = TypPobytuCis.newInstance(params)
		...
		TypPobytuCis.withTransaction {status->
			try {
	...
	@Transactional
	def update() {
		TypPobytuCis instance = TypPobytuCis.lock(params.id)
		if (!instance) {
			respond response:[status:-1,data:message(code:"default.not.found.message",args:[
					message(code:"typPobytuCis.label"),
					params.id
				])]
			return
		}
