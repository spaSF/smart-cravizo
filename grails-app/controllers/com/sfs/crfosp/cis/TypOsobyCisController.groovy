...
class TypOsobyCisController {
	...
	def index() {
		respond response:responseSCService.gridResponse(TypOsobyCis.class,params)
	}

	@Transactional
	def save() {
		TypOsobyCis instance = TypOsobyCis.newInstance(params)
		...
		TypOsobyCis.withTransaction {status->
			try {
	...
	@Transactional
	def update() {
		TypOsobyCis instance = TypOsobyCis.lock(params.id)
		if (!instance) {
			respond response:[status:-1,data:message(code:"default.not.found.message",args:[
					message(code:"typOsobyCis.label"),
					params.id
				])]
			return
		}
