...
class NarodnostCisController {
	...
	def index() {
		respond response:responseSCService.gridResponse(NarodnostCis.class,params)
	}

	@Transactional
	def save() {
		NarodnostCis instance = NarodnostCis.newInstance(params)
		...
		NarodnostCis.withTransaction {status->
			try {
	...
	@Transactional
	def update() {
		NarodnostCis instance = NarodnostCis.lock(params.id)
		if (!instance) {
			respond response:[status:-1,data:message(code:"default.not.found.message",args:[
					message(code:"narodnostCis.label"),
					params.id
				])]
			return
		}
