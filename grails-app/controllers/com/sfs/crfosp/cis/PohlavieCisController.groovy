...
class PohlavieCisController {
	...
	def index() {
		respond response:responseSCService.gridResponse(PohlavieCis.class,params)
	}

	@Transactional
	def save() {
		PohlavieCis instance = PohlavieCis.newInstance(params)
		...
		PohlavieCis.withTransaction {status->
			try {
	...
	@Transactional
	def update() {
		PohlavieCis instance = PohlavieCis.lock(params.id)
		if (!instance) {
			respond response:[status:-1,data:message(code:"default.not.found.message",args:[
					message(code:"pohlavieCis.label"),
					params.id
				])]
			return
		}
