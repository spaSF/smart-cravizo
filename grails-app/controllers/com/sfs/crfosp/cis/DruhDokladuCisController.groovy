...
class DruhDokladuCisController {
	...
	def index() {
		respond response:responseSCService.gridResponse(DruhDokladuCis.class,params)
	}

	@Transactional
	def save() {
		DruhDokladuCis instance = DruhDokladuCis.newInstance(params)
		...
		DruhDokladuCis.withTransaction {status->
			try {
	...
	@Transactional
	def update() {
		DruhDokladuCis instance = DruhDokladuCis.lock(params.id)
		if (!instance) {
			respond response:[status:-1,data:message(code:"default.not.found.message",args:[
					message(code:"druhDokladuCis.label"),
					params.id
				])]
			return
		}
